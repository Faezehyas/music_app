...
import 'package:ahanghaa/views/my_widgets/horizontal_podcast_item.dart';
...
class HorizontalPodcastsList extends StatefulWidget {
  List<PodcastModel> podcastsList = [];
  String iconUrl = '';
  String title = '';

  HorizontalPodcastsList(
      {required this.podcastsList, required this.iconUrl, required this.title});

  @override
  _HorizontalPodcastsListState createState() => _HorizontalPodcastsListState();
}

class _HorizontalPodcastsListState extends State<HorizontalPodcastsList> {
  @override
        ...
        children: [
          ...
          Container(
            ...
            child: ListView(
              ...
              children: (widget.podcastsList.length > 8
                      ? widget.podcastsList.getRange(0, 8).toList()
                      : widget.podcastsList)
                  .map((e) {
                return InkWell(
                    highlightColor: Colors.transparent.withOpacity(0),
                    splashColor: Colors.transparent.withOpacity(0),
                    onLongPress: () {
                      showOptionMenu(context, convertPodcastToMusic(e));
                    },
                    onTap: () {
                          ...
                          PageTransition(
                              ...
                              child: PlayerScreen(
                                  currentList: <MusicModel>[
                                    ...widget.podcastsList.map((e1) {
                                      return convertPodcastToMusic(e1);
                                    }).toList()
                                  ],
                                  currentMusicIndex:
                                      widget.podcastsList.indexOf(e))));
                    },
                    child: HorizontalPodcastItem(e));
              }).toList(),
            ),
          ),
        ],
