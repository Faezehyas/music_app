...
class WithoutTabPodcastListScreen extends StatefulWidget {
  String title = '';
  String iconUrl = '';

  WithoutTabPodcastListScreen(this.title, this.iconUrl);

  @override
  _WithoutTabPodcastListScreenState createState() =>
      _WithoutTabPodcastListScreenState();
}

class _WithoutTabPodcastListScreenState
    extends State<WithoutTabPodcastListScreen> {
      ...
      return Scaffold(
        body: Column(
          children: [
            ...
            Expanded(
                child: ListView(
              children: mainProvider.favoritePodcastList
                  .map((e) => InkWell(
                      highlightColor: Colors.transparent.withOpacity(0),
                      splashColor: Colors.transparent.withOpacity(0),
                      onTap: () {
  ...
  playMusic(PodcastModel podcastModel) {
        ...
        PageTransition(
            ...
            child: PlayerScreen(
                currentList: <MusicModel>[
                  ..._mainProvider.favoritePodcastList.map((e1) {
                    return convertPodcastToMusic(e1);
                  }).toList()
                ],
                currentMusicIndex:
                    _mainProvider.favoritePodcastList.indexOf(podcastModel))));
  }
}
