...
class WithoutTabListScreen extends StatefulWidget {
  String title = '';
  String iconUrl = '';
  int id = 0;

  WithoutTabListScreen(this.title, this.iconUrl, this.id);

  @override
  _WithoutTabListScreenState createState() => _WithoutTabListScreenState();
}

class _WithoutTabListScreenState extends State<WithoutTabListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer2<MainProvider, PlayerProvider>(
      ...
      if (!isLoaded) {
        ...
        if (widget.id > 0) {
          mainProvider.getAlbumsMusicList(context, widget.id).then((value) {
            setState(() {
              musicList = value;
            });
          });
